...
class CameraInfo(NamedTuple):
    ...
    width: int
    height: int
    objects: np.array # semantic related
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, objects_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        # image_name = os.path.basename(image_path).rsplit(".", 1)[0] # For TUM-RGBD
...
def readColmapSceneInfo(path, images, eval, object_path, llffhold=8):
    try:
    ...
    object_dir = "object_mask" if object_path == None else object_path # semantic related
